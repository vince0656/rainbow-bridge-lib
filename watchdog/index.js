    ...
    while (true) {
      try {
        ...
        if (Number(bridgeState.nextValidAt) == 0) {
          const delay = 10
          console.log(`No block to check. Sleeping for ${delay} seconds.`)
        ...
        for (let i = 0; i < Number(bridgeState.numBlockProducers); i++) {
          ...
          let result
          try {
            result = await this.clientContract.methods.checkBlockProducerSignatureInHead(i).call({handleRevert: true})
          } catch (e) {
